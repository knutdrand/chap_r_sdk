test_that("create_train_cli shows deprecation warning", {
  mock_train <- function(training_data, model_configuration) {
    return("test_model.rds")
  }
  ...
  expect_warning(
    create_train_cli(mock_train, args = c(temp_csv)),
    "deprecated"
  )
...
test_that("create_predict_cli shows deprecation warning", {
  mock_predict <- function(historic_data, future_data, saved_model, model_configuration) {
  ...
  temp_model <- tempfile(fileext = ".rds")
  ...
  readr::write_csv(test_data, temp_historic)
  readr::write_csv(test_data, temp_future)
  saveRDS(list(test = "model"), temp_model)

  expect_warning(
    create_predict_cli(mock_predict, args = c(temp_historic, temp_future, temp_model)),
    "deprecated"
  )

  unlink(c(temp_historic, temp_future, temp_model))
})
