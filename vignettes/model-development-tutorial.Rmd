...
predict_fn <- function(historic_data, future_data, saved_model,
  ...
  # Returns: tibble with either:
  #   - disease_cases column (point predictions), or
  #   - samples list-column (probabilistic predictions with Monte Carlo samples)
}
```
...
Your predictions must include either a `disease_cases` column (point predictions) or a `samples` list-column (probabilistic predictions).
...
That's it! Just 12 lines of actual model logic. We convert to tibble before grouping to avoid tsibble key conflicts.
...
## Probabilistic Predictions (Optional)

For probabilistic forecasting, return Monte Carlo samples instead of point predictions:
...
The `samples` column is a list-column where each element is a numeric vector of samples.
