...
Your predictions must include a `samples` list-column. For deterministic models, use a single sample per row.
...
That's it! We convert to tibble before grouping to avoid tsibble key conflicts, and wrap each prediction in `list(c(value))` to create the samples column.
