...
before forecasting. See `examples/arima_model/` for a demonstration of
this pattern.
...
Note: We use `as_tibble()` in the training function because
`summarise(.by = ...)` needs a tibble to collapse across the time
dimension. The prediction function works directly on tsibbles since
`left_join()` and `mutate()` preserve tsibble structure.
...
2.  **Validate** with stubs using `validate_model_io()` to understand
    requirements
