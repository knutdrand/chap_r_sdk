...
#' @param include_chap_sdk Whether to include chapr in dependencies (default: TRUE)
...
init_chap_env <- function(r_version = NULL, include_chap_sdk = TRUE) {
  ...
  # 2. Add chapr to dependencies if requested
  if (include_chap_sdk) {
    message("Adding chapr to dependencies...")
    tryCatch({
      renv::install("chapr")
    }, error = function(e) {
      message("Note: chapr not found in repositories. ",
...
generate_dockerfile_content <- function(base_image,
  ...
  paste0(
    "# Generated by chapr::generate_dockerfile()\n",
